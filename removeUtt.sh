...
# 加上：去掉长度较短的句子后，所有句子数小于limit_utts的spk对应的句子
# 有重复，但没关系
if [ "$limit_utts" -gt 0 ]; then
  utt=$utt`echo $utt | awk -v limit=$limit_utts '
    NR==FNR{for(i=1;i<=NF;i++){
      a[$i]=1;}}
    NR>FNR{tot=NF-1; for(i=2;i<=NF;i++){
      if(a[$i]==1){
        tot=tot-1;
      }}
      if(tot<limit){
...
# fix_data_dir.sh 最后调用了 utt2spk_to_spk2utt.pl
echo "$data/spk2utt done"

if [ -f $data/spk2gender ];then
  ${SUBTOOLS}/kaldi/utils/filter_scp.pl $data/spk2utt $data/spk2gender > $data/spk2gender.tmp && \
    mv -f $data/spk2gender.tmp $data/spk2gender
