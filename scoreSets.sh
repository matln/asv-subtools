...
lda_data_config="trainset[trainset devset]"  # if NULL, will be set "$enrollset[$enrollset $testset]"
...
submean_data_config="trainset[trainset devset]" # if NULL, will be set "$enrollset[$enrollset $testset]"
...
whiten_data_config="trainset[trainset devset]" # if NULL, will be set "$enrollset[$enrollset $testset]"
...
process_force_clear=true  # true: will "rm -f $vectordir/$enrollset/num_utts.ark"
trials_force_clear=false  # true: 若 $trials==""，会使用 subtools/getTrials.sh 自动生成trils文件
score_force_clear=true    # true: line 331，强制重新计算得分
...
#   vectordir         包含 vector (ivector, xvector)、log、config 等文件或文件夹
#   vectorfile        vector 文件的文件名
...
for set in $(echo "$allsets" | sed 's/ /\n/g' | sed '/^$/d' | sort -u); do
  ...
  [[ "$errorNum" -gt 0 ]] && echo "There are some ERRORS in $vectordir/$set/log/*.log and it means you \
    lose many vectors which is so bad thing and I suggest you to extract vectors of this dataset again." && exit 1
  echo -e "name $set\ndata data/$prefix/$set\nvectordir $vectordir/$set\nvectorfile $vectortype.scp" > $vectordir/"$set"/config
done
...
[ "$plda_trainset" != "" ] && writeconf plda_data_conf $vectordir/$plda_trainset/config $vectordir/$plda_trainset/config
...
# process.sh function process() 144 line
# a file which containg the output file of the process that has been exected
processed="$vectordir/$testset/processed.list" # a global file which is used to avoiding re-computation
> $processed
