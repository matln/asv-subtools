...
class FocalLoss(TopVirtualLoss):
    ...
    def forward(self, inputs, targets):
        ...
        focal_posterior = (1 - posterior)**self.gamma * \
            torch.log(posterior.clamp(min=self.eps))
...
class MarginSoftmaxLoss(TopVirtualLoss):
    ...
    def init(self, input_dim, num_targets,
        ...
        if feature_normalize:
            ...
            suggested_s = [(num_targets - 1) / num_targets * np.log(
                (num_targets - 1) * x / (1 - x)) for x in p_target]
    ...
    def forward(self, cosine_theta, cosine_theta_target, penalty_cosine_theta):
        with torch.no_grad():
            self.t = (1 - self.momentum) * \
                cosine_theta_target.mean() + self.momentum * self.t
...
class LogisticAffinityLoss(TopVirtualLoss):
    ...
    def forward(self, inputs, targets):
        ...
        if self.reduction == 'sum':
            ...
            raise ValueError(
                "Do not support this reduction {0}".format(self.reduction))
    ...
    def get_accuracy(self, targets):
        if self.training:
            ...
            return lam * self.compute_accuracy(self.base_loss.get_posterior(), targets) + \
                (1 - lam) * self.compute_accuracy(self.base_loss.get_posterior(),
                                                  targets[index])
        else:
