...
class TdnnAffine(torch.nn.Module):
    ...
    def __init__(self, input_dim, output_dim, context=[0], bias=True, stride=1, pad=True, groups=1, norm_w=False, norm_f=False):
        ...
        self.weight = torch.nn.Parameter(torch.randn(
            output_dim, input_dim // groups, *kernel_size))
    ...
    def forward(self, inputs):
        ...
        if not self.selected_device and self.mask is not None:
            ...
            # Another simple case, for a temporary tensor, just specify the device when creating it.
...
class TdnnfBlock(torch.nn.Module):
    ...
    def __init__(self, input_dim, output_dim, inner_size, context_size=0, pad=True):
        ...
        self.factor1 = TdnnAffine(
            input_dim, inner_size, context_factor1, pad=pad, bias=False)
        self.factor2 = TdnnAffine(
            inner_size, output_dim, context_factor2, pad=pad, bias=True)
...
class MultiAffine(torch.nn.Module):
    ...
    def __init__(self, input_dim, output_dim, num=1, split_input=True, bias=True):
        ...
        self.weight = torch.nn.Parameter(torch.randn(
            1, self.num, self.output_dim, self.num_feature_every_part))

        if self.bool_bias:
            self.bias = torch.nn.Parameter(
                torch.randn(1, self.num, output_dim, 1))
        else:
            self.register_parameter('bias', None)
