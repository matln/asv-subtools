...
path = os.popen('echo $KALDI_ROOT/src/bin:$KALDI_ROOT/tools/openfst/bin:$KALDI_ROOT/src/fstbin/:'
                '$KALDI_ROOT/src/gmmbin/:$KALDI_ROOT/src/featbin/:$KALDI_ROOT/src/lm/:'
                '$KALDI_ROOT/src/sgmmbin/:$KALDI_ROOT/src/sgmm2bin/:$KALDI_ROOT/src/fgmmbin/:'
                '$KALDI_ROOT/src/latbin/:$KALDI_ROOT/src/nnetbin:$KALDI_ROOT/src/nnet2bin:'
                '$KALDI_ROOT/src/nnet3bin:$KALDI_ROOT/src/online2bin/:$KALDI_ROOT/src/ivectorbin/:'
    ...
    try:
        ...
        if re.search('^(ark|scp)(,scp|,b|,t|,n?f|,n?p|,b?o|,n?s|,n?cs)*:', file):
            ...
            if prefix != "scp" and prefix != "ark":
                raise TypeError(
                    "Specifier of {} is not scp or ark.".format(file))
        if prefix is None:
            raise TypeError(
                "Specifier of {} is None, please specify this with scp or ark.".format(file))
        # separate offset from filename (optional),
        if re.search(':[0-9]+$', file):
            (file, offset) = file.rsplit(':', 1)
        # input pipe?
...
def read_key(fd):
    ...
    assert(re.match('^\S+$', key) is not None)  # check format (no whitespace!)
    return key
...
def _read_mat_binary(fd, chunk=None):
    ...
    if start_index > 0:
        ...
        fd.seek(start_index * cols * sample_size, 1)
...
def read_cntime(file_or_fd):
    ...
    data = np.frombuffer(fd.read(vec_size * 10), dtype=[
        ('size_beg', 'int8'), ('t_beg', 'float32'), ('size_end', 'int8'), ('t_end', 'float32')], count=vec_size)
