...
class ChunkEgs(Dataset):
    """Prepare chunk-egs for time-context-input-nnet (such as xvector which transforms egs fromm chunk-frames level to
    ...
    def __init__(self, egs_csv, io_status=True, aug=None, aug_params={}):
        ...
        # For multi-label.
        # Except utt-id, path, chunk-start and chunk-end.
        self.num_target_types = self.data_frame.shape[1] - 4
    ...
    def __getitem__(self, index):
        ...
        # 2020.7.28 22.52 lijianchen
        # egs = egs.astype(np.float32)
...
class VectorEgs(Dataset):
    ...
    def __init__(self, egs_csv, io_status=True, aug=None, aug_params={}):
        ...
        self.data_frame = pd.read_csv(egs_csv, sep=" ").values
        # For multi-label.
        # Except utt-id and path.
        self.num_target_types = self.data_frame.shape[1] - 2
    ...
    def __getitem__(self, index):
        ...
        egs = np.require(kaldi_io.read_vec_flt(
            self.data_frame[index][1]), requirements=['O', 'W'])
...
class BaseBunch():
    ...
    def get_bunch_from_csv(cls, trainset_csv: str, valid_csv: str = None, egs_params: dict = {}, data_loader_params_dict: dict = {}):
        ...
        if "egs_type" in egs_params.keys():
            ...
            if egs_type == "chunk":
                ...
                raise TypeError(
                    "Do not support {} egs now. Select one from [chunk, vector].".format(egs_type))
    ...
    @classmethod
    def get_bunch_from_egsdir(cls, egsdir: str, egs_params: dict = {}, data_loader_params_dict: dict = {}):
        feat_dim, num_targets, train_csv, valid_csv = _get_info_from_egsdir(egsdir)
        info = {"feat_dim": feat_dim, "num_targets": num_targets}
        bunch = cls.get_bunch_from_csv(train_csv, valid_csv, egs_params, data_loader_params_dict)
        return bunch, info
...
def _get_info_from_egsdir(egsdir):
    if os.path.exists(egsdir + "/info"):
        ...
        valid_csv = egsdir + "/validation.egs.csv"
